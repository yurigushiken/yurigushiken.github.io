...
This project grew out of a question from our EEG work on numerical cognition: instead of looking only at averaged signals, could we use machine learning to compare how different numerosities are represented relative to one another across the full spatiotemporal EEG pattern? I wanted an approach that stayed close to the trial-level data while still giving us a way to think about the structure of the representation space, not just whether one condition was bigger or smaller than another.

To do that, I built a workflow around pairwise decoding and representational similarity analysis. The practical challenge was not just training models, but doing it in a way that respected subject boundaries, controlled obvious visual confounds, and gave outputs that were interpretable enough to be useful for research rather than just technically impressive. A large part of the work was deciding what to standardize, what to hold out, and how to keep the analysis reproducible from run to run.
